...
/// </summary>
    public partial class Form1 : Form
    {
       ...
       private AdsStream dataStream;
        ...
        "MAIN.S1","MAIN.S2","MAIN.ALLARME","MAIN.T_ALLARME","MAIN.PORTA","MAIN.PORTACHIUSA"};
        private int NUM_ELEM_BOOL = 27;
        ...
        private void CodificaComandiBraccioRobot()
        {
            ...
            bool allungato1 = false;
            bool allungato3 = false;
        ...
        private void GestioneBracci()
        {
            ...
            switch (numerobraccio)
            {
                ...
                case -1 when posyRobot != posYinizialeRobot:
                    braccio1.Location = new Point(robot.Location.X + 26, posy - 4);
        ...
        private void spostaVestitob1(PictureBox vestito)
        {
            ...
            if (vestito != null)
            {
                if (Braccio1Carico && vestito.Bounds.IntersectsWith(braccio1.Bounds))
                {
                ...
                else if (Braccio1Carico && !vestito.Bounds.IntersectsWith(braccio1.Bounds) /*&& vestito.Bounds.IntersectsWith(robot.Bounds)*/)
                {
                    vestito.Location = new Point(robot.Location.X + 20, robot.Location.Y - 25);
                }
        ...
        private void spostaVestitob2(PictureBox vestito1)
        {
            ...
            if (vestito1 != null)
            {
                if (Braccio3Carico && vestito1.Bounds.IntersectsWith(braccio3.Bounds))
                {

                    vestito1.Location = new Point(braccio3.Location.X - 5, braccio3.Location.Y + 7);

                }
                else if (Braccio3Carico && !vestito1.Bounds.IntersectsWith(braccio3.Bounds))
                {
                    vestito1.Location = new Point(robot.Location.X - 5, robot.Location.Y + 40);
                }
        ...
        private void cONNECT_Click(object sender, EventArgs e)
        {
            ...
            hConnect[26] = tcClient.AddDeviceNotification(dataPLC[26], dataStream, 26, 1, AdsTransMode.OnChange, 100, 0, TextApriPorta);
            hConnect[27] = tcClient.AddDeviceNotification(dataPLC[27], dataStream, 27, 1, AdsTransMode.OnChange, 100, 0,textFcsPorta);
        ...
        private void OnNotification(object sender, AdsNotificationEventArgs e)
        {
            ...
            if (e.NotificationHandle == hConnect[26])
                strValue = binRead.ReadBoolean().ToString();
            if (e.NotificationHandle == hConnect[27])
                strValue = binRead.ReadBoolean().ToString();
        ...
        private void APRIPORTA_Click(object sender, EventArgs e)
        {

        }

        private void TextApriPorta_TextChanged(object sender, EventArgs e)
        {
            if (hvar_name != null)
            {
                if (TextApriPorta.Text.Equals("True"))
                    tcClient.WriteAny(hvar_name[26], true);
                if (TextApriPorta.Text.Equals("False"))
                    tcClient.WriteAny(hvar_name[26], false);
            }
        }

        private void textFcsPorta_TextChanged(object sender, EventArgs e)
        {
            if (hvar_name != null)
            {
                if (textFcsPorta.Text.Equals("True"))
                    tcClient.WriteAny(hvar_name[27], true);
                if (textFcsPorta.Text.Equals("False"))
